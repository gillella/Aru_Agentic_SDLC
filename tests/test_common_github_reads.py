...
import github_inventory  # noqa: E402
...
class RestIssueInventoryTests(unittest.TestCase):
    ...
    @patch.object(common, "_issue_trust_identity")
    @patch.object(common, "get_repo_slug", return_value="acme/widgets")
    @patch.object(common, "run_gh_json")
    def test_trusted_rest_association_skips_graphql(self, gh_json, _slug, trust):
        gh_json.return_value = self.issue()
    ...
    @patch.object(common, "get_repo_slug", return_value="acme/widgets")
    @patch.object(common, "run_gh_json")
    def test_outsider_uses_conditional_editor_lookup(self, gh_json, _slug):
        outsider = self.issue()
        outsider["author_association"] = "NONE"
        gh_json.side_effect = [outsider, {"data": {"repository": {"issue": {
            "editor": {"login": "owner"}, "authorAssociation": "COLLABORATOR",
        }}}}]
