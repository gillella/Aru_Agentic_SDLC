...
@pytest.fixture
def config(tmp_path):
    value = json.loads((ROOT / 'integrations/chopin/config.example.json').read_text())
...
def test_real_hermes_plugin_and_telegram_command_path(config, tmp_path):
    ...
    result = subprocess.run([str(python), str(ROOT / 'integrations/chopin/hermes_probe.py')], cwd=home,
