...
FORBIDDEN_CONTENT_RE = re.compile(
    ...
    r"|^\s*\{\s*[\"']?role[\"']?\s*:\s*[\"']?(?:system|developer|user|assistant|human)\b"
    r"|^\s*-?\s*role\s*[:=]\s*(?:system|developer|user|assistant|human)\b"
    r"|<\|(?:system|developer|user|assistant|human)\|>|<\|im_(?:start|end)\|>"
    r"|^\s*<\|im_start\|>\s*(?:system|developer|user|assistant|human)\b"
    r"|^\s*(?:<<\/?SYS>>|\[/?INST\])"
    r"|^\s*(?:###\s*)?(?:system|developer|user|assistant|human|instructions)\s*(?::|$)"
