...
from secure_json import (  # noqa: F401  (re-exported for pre-deletion callers)
    SECRET_RE,
    RegistryError,
    _file_lock,
    _private_directory,
...
# Private atomic JSON storage lives in secure_json.py so durable state outlives
# the Slack runtime (#411). These names are re-exported unchanged so the
# still-live Slack and presence callers keep working until their own deletion
# slices; the re-exports go with them. RegistryError must stay the *same* class
# object here, because delivery_increments.IncrementError subclasses it.
