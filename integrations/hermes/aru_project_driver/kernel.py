...
class _Bridge:
    ...
    def _pr(self, pr: dict, linked_records: dict[int, dict]) -> dict:
        ...
        authors = []
        if len(linked) == 1:
            issue = linked_records.get(linked[0])
            if not isinstance(issue, dict) or not isinstance(issue.get("labels"), list):
                raise KernelAdapterError("linked issue claimant evidence is unavailable")
            authors = [name.removeprefix("agent:") for name in self.common.label_names(issue)
                       if name.startswith("agent:")]
            if len(authors) > 1:
                raise KernelAdapterError("linked issue has contradictory claimant identities")
            for author in authors:
                self.claims.safe_agent(author)
