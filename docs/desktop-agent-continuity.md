...
The retained client-work kernel exposes run health through
`scripts/fleet_status.py`, `scripts/loop_control.py`, and
`scripts/agent_presence.py`. That surface is read-only with respect to issue
and PR authority: it reports current holders, review wait, CI wait, Ready
depth, and pause state without becoming a second claim source, scheduler, or
queue. `loop_control.py` remains the bounded pause/resume surface, and
`agent_presence.py` remains the retained agent liveness signal.
