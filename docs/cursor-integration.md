...
- `init-agent-project`
- `create-github-issue`
- `code-review` (only for an explicitly preassigned emergency `review:agent` fallback)
...
pick feedback → merge → issue; external review is normal, and only an explicit
emergency `review:agent` assignment becomes resumable review work. It never
creates a coding-agent review queue. Do not route bare "continue" to
