...
A live smoke test is a separate, operator-authorized exercise and must never
target this repository or an existing project board:
...
5. Observe claim, worktree, PR, CI, independent review, guarded merge, Done
   ...
   issue/PR URLs and gate outcomes. Stop each operator-started worker task,
   then run the authoritative final audit:
...
paid desktop-agent sessions and mutates a real GitHub repository. Run it only
